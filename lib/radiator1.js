 ...
 */
    ...
    _sendCommand(deviceId, command, setpoint, callback) {
        const self = this,
            ...
            buffer = [0x0C, defines.RADIATOR1, self.subtype, seqnbr,
                      device.idBytes[0], device.idBytes[1], device.idBytes[2], device.idBytes[3], device.unitCode,
    ...
    setTemperature(deviceId, temperature, callback) {
        ...
        let setpoint = Math.max(Math.min(28.0, temperature), 5.0);
        setpoint = Math.round(2*setpoint)/2;
        setpoint = [Math.floor(setpoint), 10*(setpoint - Math.floor(setpoint))];
        return this._sendCommand(deviceId, 0x02, setpoint, callback);
    }
