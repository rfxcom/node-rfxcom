 ...
 */
    ...
    _sendCommand(deviceId, command, level, callback) {
        ...
        let buffer = [0x0a, defines.LIGHTING5, self.subtype, seqnbr, device.idBytes[0], device.idBytes[1],
                      device.idBytes[2], device.unitCode, command, level, 0];
 ...
 */
    switchOn(deviceId, options, callback) {
        let command, level;
 ...
 */
    increaseLevel(deviceId, callback) {
 ...
 */
    decreaseLevel(deviceId, callback) {
        if (this.isSubtype(["MDREMOTE", "LIVOLO", "TRC02", "TRC02_2", "RGB432W", "MDREMOTE_108"])) {
