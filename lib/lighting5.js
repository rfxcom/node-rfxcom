...
Lighting5.prototype._splitDeviceId = function(deviceId) {
    ...
    var idBytes = this.rfxcom.stringToBytes(parts[0]);
    if (idBytes.length !== 3) {
        throw new Error("Invalid deviceId format");
    }
    var idValue = (idBytes[0]*256 + idBytes[1])*256 + idBytes[2];
    if (idValue < 1 || ((this.subtype == 0 || this.subtype == 4 || this.subtype == 6) && idValue > 0xfffff) ||
        ...
        throw new Error("Address outside valid range");
    }
    ...
    if (unitCode < 1 || ((this.subtype == 0 || this.subtype == 4 ) && unitCode > 16) ||
        ...
        throw new Error("Unit code outside valid range");
    }
    return {
        idBytes: idBytes,
        unitCode: unitCode
    ...
    if (device.unitCode == 0) {
        if (self.subtype == 0x00 || self.subtype == 0x02 || self.subtype == 0x04) {
            if (command == 0x00) {
                ...
                return 0;           // LightwaveRF doesn't support group On
            }
        } else {
            return 0;               // Subtype doesn't support group commands
        }
    }
