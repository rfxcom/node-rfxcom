...
*/
    _splitDeviceId(deviceId) {
        ...
        } else if (this.isSubtype(["QH_A19", "BYRON_DBY"])) {
        ...
        if ((this.isSubtype("BYRON_SX") && id.value > 0xff) ||
            ...
            (this.isSubtype("QH_A19") && (id.value > 0xffffff03 || id.bytes[3] > 3)) ||
            (this.isSubtype("BYRON_DBY") && id.value > 0xffffffff)) {
    ...
    _sendCommand(deviceId, command, callback) {
        ...
        if (this.isSubtype(["QH_A19", "BYRON_DBY"])) {
            var buffer = [device.idBytes[0], device.idBytes[1], device.idBytes[2], device.idBytes[3], 0];
        } else {
            var buffer = [device.idBytes[0], device.idBytes[1], device.idBytes[2], 0];
        }   
