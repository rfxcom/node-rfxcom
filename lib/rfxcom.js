    ...
    _transform(buffer, encoding, callback) {
        if (this.rfxtrx.receiving) {
            ...
            while (this.data.length >= this.requiredBytes) {
                ...
                if (this.data.length > 0 && this.data[0] >= 4 && this.data[0] <= 255) {
...
class RfxCom extends EventEmitter {
    constructor(device, options) {
        ...
        // Create the parser and listen for data events from it (the parser will be applied to the serialport once it created)
    ...
    statusMessageHandler(data) {
        ...
        if (subtype === 0xFF) {         // Message not understood!
            ...
            if (self.initialising) {
                //if (self.startReceiverRequired === false) {
            ...
            receiverType = receiverTypes[msg[1]] || "Unknown device";
