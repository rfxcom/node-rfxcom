...
function RfxCom(device, options) {
    ...
    self.on("ready", function() {
        self.reset(function() {
            self.delay(500);
            self.flush(function() {
                self.getStatus(self.readyCallback);
            });
        });
    });
...
RfxCom.prototype.open = function() {
    ...
    if (typeof self.serialport === "undefined" || self.fakingSerialPort) {
        ...
        self.serialport.on("error", function(msg) {
            ...
            // destroy the existing serialport and emit the disconnect event so a host application can
            // attempt to reconnect (when the RFXtrx433 is plugged in again)
            console.log("[rfxcom] on " + self.device + " - " + msg);
            if (msg.code === "ENXIO" || (msg.message && msg.message.indexOf("Cannot open") >= 0)) {
                self.close();
