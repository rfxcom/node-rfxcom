        ...
        if (this.rfxtrx.receiving) {
            ...
            while (this.data.length >= this.requiredBytes) {
                if (this.requiredBytes > 0) {
                    this.push(Buffer.from(this.data.slice(0, this.requiredBytes)));
    ...
    open() {
        ...
        if (typeof self.serialport === "undefined") {
            ...
            self.serialport = new SerialPort(self.device, {
                baudRate: 38400,
/*
                Serialport 6 uses Transform parser
...
//        self.serialport.on("data", function(data) {
        self.parser.on("data", function(buffer) {
                let data = [];
                data.push.apply(data, buffer);
                const
                    length = data[0] + 1,
...
Serialport 6 does not implement the disconnect event  - uses "close" instead
        self.serialport.on("disconnect", disconnectHandler);
*/
        self.serialport.on("close", disconnectHandler);
