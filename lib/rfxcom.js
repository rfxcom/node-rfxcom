...
class RfxCom extends EventEmitter {
    constructor(device, options) {
        ...
        self.packetNames = rfxcom.packetNames;
        ...
        self.parser = new RFXParser(self);
    ...
    open() {
        ...
        // Add serialport parser data event listener
        ...
        const disconnectHandler = function(err) {
            // Call close() to clear the transmit queue and reset our state, then emit the
            // appropriate event
            const wasConnected = self.connected;
            self.close();
            if (wasConnected) {
                self.emit("disconnect", err.message);
            } else {
                self.emit("connectfailed", err.message);
            }
        };

        self.serialport.on("error", (err) => {
            self.debugLog(err.message);
        ...
        self.serialport.on("close", (err) => {
            // Handle a disconnect when one occurs
            if (err && err.disconnected === true) {
                self.debugLog("RFXtrx433 disconnected from " + self.device);
                disconnectHandler(err);
            }
        });
