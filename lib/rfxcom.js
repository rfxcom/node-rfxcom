    ...
    statusMessageHandler(data) {
        ...
        if (subtype === 0xFF) {         // Message not understood!
            ...
            if (data.length > 12) {
                ...
                msg = data.slice(2, 19);
                firmwareType = msg[10];
                // 'Next-generation' non-FTDI chip devices are now being using 2000 series firmware
 ...
 */
    fanHandler(data, packetType) {
        ...
        switch (evt.subtype){
            ...
            case 12:
                evt.id = "0x" + RfxCom.dumpHex(data.slice(2, 5), false).join("");
                evt.command = [undefined, "Low", "Medium", "High", "Timer 1", "Timer 2", "Timer 3", "Auto",
                    "Away", "Join", "", "State", "Temperature", "CO2", "Battery", "Filter", "Presence", "Speed",
                    "Status"][evt.commandNumber];
                if (data.length === 16) {
...
activLinkHandler(data, packetType) {
    ...
    if (data[0] === 0x00) {
        ...
        evt.command = ["Normal", "Secret Knock"][data[6]] || ""
    } else if (data[0] === 0x01) {
 ...
 */
    blinds2Handler(data, packetType) {
        ...
        evt = {
            ...
            id: "0x" + RfxCom.dumpHex(data.slice(2, 6), false).join(""),
            commandNumber: data[7],
            command: ["Up", "Down", "Stop", "Pair", "Set Percent", "Set Angle", "Set Percent and Angle"][data[7]] || "undefined",
            percent: data[8],
            angle: data[9],
