...
class Thermostat4 extends Transmitter {
    ...
    _splitDeviceId(deviceId) {
        ...
        let id = RfxCom.stringToBytes(parts[0], 3);
    ...
    _sendCommand(deviceId, beep, fan1Speed, fan23Speed, flamePower, mode, callback) {
        const self = this,
            ...
            buffer = [0x0c, defines.THERMOSTAT4, self.subtype, seqnbr,
                        device.idBytes[0], device.idBytes[1], device.idBytes[2],
                        beep, fan1Speed, fan23Speed, flamePower, mode, 0];
        self.rfxcom.queueMessage(self, buffer, seqnbr, callback);
        return seqnbr;
    };

/*
    Send the message with the specified parameters, given by the fields of the param object:
    {
