 ...
 */
    _splitDeviceId(deviceId) {
        ...
        if (this.isSubtype(["BLINDS_T2", "BLINDS_T4", "BLINDS_T5", "BLINDS_T10", "BLINDS_T18"])) {
            unitCode = 0;
        } else if (parts.length === 2) {
            unitCode = parseInt(parts[1]);
            if (isNaN(unitCode) || unitCode < 0) {
                throw new Error("Invalid unit code " + parts[1]);
            } else if (this.isSubtype(["BLINDS_T0", "BLINDS_T1", "BLINDS_T6", "BLINDS_T7", "BLINDS_T19"])) {
                if (unitCode > 15) {
                    throw new Error("Invalid unit code " + parts[1]);
                }
                ...
                unitCode = unitCode - 1;
            } else if (this.isSubtype(["BLINDS_T9", "BLINDS_T11", "BLINDS_T16"])) {
                if (unitCode > 6) {
                    throw new Error("Invalid unit code " + parts[1]);
                }
            ...
            } else if (this.isSubtype("BLINDS_T20")) {
                if (unitCode === 0) {
                    throw new Error("Subtype doesn't support group commands");
                } else if (unitCode > 9) {
                    throw new Error("Invalid unit code " + parts[1]);
        ...
        if ((id.value === 0) ||
            ((this.isSubtype(["BLINDS_T0", "BLINDS_T1", "BLINDS_T12", "BLINDS_T13",
                "BLINDS_T15", "BLINDS_T16", "BLINDS_T19", "BLINDS_T20"])) && id.value > 0xffff) ||
 ...
 */
    open(deviceId, callback) {
        if (this.isSubtype("BLINDS_T19")) {
            throw new Error("Device does not support open()");
        } else {
 ...
 * Close deviceId.
 */
    close(deviceId, direction, callback) {
        if (callback === undefined && typeof direction === "function") {
            callback = direction;
            direction = "CW"
        }
        let command = defines.BLINDS_CLOSE;
        if (this.isSubtype("BLINDS_T19")) {
            if (direction === "CW") {
                command = defines.BLINDS_T19_CLOSE_CW;
            } else if (direction === "CCW") {
                command = defines.BLINDS_T19_CLOSE_CCW;
            } else {
                throw new Error("Close direction must be 'CW' or 'CCW")
            }
        }
 ...
 */
    stop(deviceId, callback) {
        if (this.isSubtype(["BLINDS_T13", "BLINDS_T19"])) {
 ...
 */
    ...
    confirm(deviceId, callback) {
        ...
        } else if (this.isSubtype("BLINDS_T19")) {
            return this._sendCommand(deviceId, defines.BLINDS_T19_CONFIRM, callback);
        } else {
    ...
    intermediatePosition(deviceId, position, callback) {
        ...
        if (this.isSubtype("BLINDS_T9")) {
            ...
            if (typeof position != "number" || isNaN(position) ||
                !(position === 45 || position === 90 || position === 135)) {
                throw new Error("Invalid position: value must be 45, 90, or 135");
            } else {
                return this._sendCommand(deviceId, 0x01 + position/45, callback);
            }
